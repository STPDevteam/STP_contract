const StandardCompliantTokenMock = artifacts.require('./mocks/StandardCompliantTokenMock')

contract('StandardCompliantToken', ([sender, recipient, ...accounts]) => {
  ...
  let SUCCESS_MESSAGE
  before(async () => {
    token = await StandardCompliantTokenMock.new(initialAccount, initialBalance)
  ...
  it('should detect success for valid transfer', async () => {
    const code = await token.checkTransferValid(sender, recipient, transferValue)
